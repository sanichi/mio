...
describe Position do
  ...
  context "invalid pieces" do
    ...
    it "bad count" do
      ...
      expect(position.errors[:pieces].first).to eq t("position.error.badc")
    end

    it "no black king" do
      position = build(:position, pieces: start.sub("k", "q"))
      expect(position).to_not be_valid
      expect(position.errors[:pieces].first).to eq t("position.error.nobk")
    end

    it "no white king" do
      position = build(:position, pieces: start.sub("K", "1"))
      expect(position).to_not be_valid
      expect(position.errors[:pieces].first).to eq t("position.error.nowk")
    end

    it "pawn on the eighth rank" do
      position = build(:position, pieces: start.sub("r", "p"))
      expect(position).to_not be_valid
      expect(position.errors[:pieces].first).to eq t("position.error.pote")
    end

    it "pawn on the first rank" do
      position = build(:position, pieces: start.sub("N", "P"))
      expect(position).to_not be_valid
      expect(position.errors[:pieces].first).to eq t("position.error.potf")
    end

    it "too many black kings" do
      position = build(:position, pieces: start.sub("r", "k"))
      expect(position).to_not be_valid
      expect(position.errors[:pieces].first).to eq t("position.error.tmbk")
    end

    it "too many white kings" do
      position = build(:position, pieces: start.sub("8/P", "7K/P"))
      expect(position).to_not be_valid
      expect(position.errors[:pieces].first).to eq t("position.error.tmwk")
    end
  end
