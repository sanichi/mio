class Aoc::Y2020d20 < Aoc
  ...
  def answer(part)
    ...
    return corners.map(&:id).reduce(:*) if part == 1
    ...
    begin
      ...
      while corners_found < 4
        ...
        unless found_next_row
          raise "FAILED to find next row for tile #{current_tile.id}"
        end
      end
      ...
      map.arrangements.each do |arr|
        ...
        next if count == 0
        # part 2
        return arr.grid.flatten.sum - MONSTER.flatten.sum * count
        break
      end
    rescue => e
      return e.message
