    ...
    begin
      ...
      file.each_line do |line|
        ...
        begin
          ...
          when "top"
            sample = line.truncate(40) # for logging, if we need to, otherwise top lines can be very long
            ...
            procs.each do |proc|
              raise E, "line #{num} (#{sample}) of #{path} has an unparsable proc:\n#{proc}" unless proc.match(/\A([1-9]\d*)\|(\d+)\|(.+)\z/)
              top.procs.create!(pid: $1, mem: $2, command: $3)
              journal.procs_count += 1
            end
          when "cpu"
            sample = line.truncate(40) # for logging, if we need to, otherwise cpu lines can be very long
