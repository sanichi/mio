...
Elm.Misc.make = function (_elm) {
   "use strict";
   _elm.Misc = _elm.Misc || {};
   if (_elm.Misc.values) return _elm.Misc.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   var updateAndDeleteUrl = function (id) {
      return A2($Basics._op["++"],
      "/todos/",
      A2($Basics._op["++"],$Basics.toString(id),".json"));
   };
   ...
   var i18Priorities = $Dict.fromList(_U.list([{ctor: "_Tuple2"
                                               ,_0: 0
                                               ,_1: "Urgent"}
   ...
   var lowPriority = 4;
   var highPriority = 0;
   return _elm.Misc.values = {_op: _op
...
Elm.Util.make = function (_elm) {
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var nbsp = $String.fromList(_U.list([$Char.fromCode(160)]));
   ...
   var formContentType = _U.list([{ctor: "_Tuple2"
                                  ,_0: "Content-Type"
                                  ,_1: "application/x-www-form-urlencoded"}]);
   var postRequest = F2(function (url,body) {
      return {verb: "POST"
...
Elm.Todo.make = function (_elm) {
   ...
   var descriptions = $Signal.mailbox({ctor: "_Tuple2"
                                      ,_0: 0
                                      ,_1: ""});
   var edits = $Signal.mailbox({ctor: "_Tuple2",_0: 0,_1: false});
   ...
   var deleteButton = function (t) {
      ...
      _U.list([$Html$Attributes.$class("btn btn-danger btn-xs")
              ,A2($Html$Events.onClick,confirmations.address,t.id)]),
      _U.list([$Html.text($Misc.i18Delete)]));
   };
   var confirmButton = function (id) {
      return A2($Html.span,
      _U.list([$Html$Attributes.$class("btn btn-danger btn-xs")
              ,A2($Html$Events.onClick,deletes.address,id)]),
      _U.list([$Html.text($Misc.i18Confirm)]));
   };
   ...
   _U.list([$Html$Attributes.$class("btn btn-default btn-xs")
           ...
           {ctor: "_Tuple0"})]),
   _U.list([$Html.text($Misc.i18Cancel)]));
   var canIncreaseDecrease = F2(function (t,bool) {
      return bool ? _U.cmp(t.priority,
      $Misc.highPriority) > 0 : _U.cmp(t.priority,
      $Misc.lowPriority) < 0;
   });
   var increaseDecreaseSpanClass = F2(function (t,bool) {
      var canChange = A2(canIncreaseDecrease,t,bool);
      var butColor = canChange ? "info" : "default";
      return $Html$Attributes.$class(A2($Basics._op["++"],
      "btn btn-",
      A2($Basics._op["++"],butColor," btn-xs")));
   });
   var priorityDescription = function (t) {
      return A2($Maybe.withDefault,
      "Unknown",
      A2($Dict.get,t.priority,$Misc.i18Priorities));
   };
   var todoCompare = F2(function (t1,t2) {
      return _U.eq(t1.id,0) || _U.eq(t2.id,0) ? A2($Basics.compare,
      t2.id,
      t1.id) : A2($Basics.xor,
      t1.done,
      ...
      t1.description,
      t2.description) : A2($Basics.compare,t1.priority,t2.priority);
   });
   ...
   var deleteTodo = function (id) {
      ...
      else return $Task.fail($Http.UnexpectedPayload("can\'t delete todos unless ID > 0"));
   };
   var updateBody = function (t) {
      ...
      _U.list([A2($Basics._op["++"],
              ...
              ,A2($Basics._op["++"],"todo%5Bdone%5D=",t.done ? "1" : "0")
              ,"_method=patch"
              ,"commit=Save"
              ,"utf8=✓"])));
   };
   var createBody = function (t) {
      return $Http.string(A2($String.join,
      "&",
      _U.list([A2($Basics._op["++"],
              "todo%5Bdescription%5D=",
   ...
   var exampleTodo = {description: ""
                     ,done: false
                     ,id: 0
                     ,priority: $Misc.highPriority
                     ,editingDescription: false
                     ,newDescription: ""};
   var creates = $Signal.mailbox(exampleTodo);
   var updates = $Signal.mailbox(exampleTodo);
   var increaseDecreaseClickHandler = F2(function (t,bool) {
      var newTodo = _U.update(t,
      {priority: t.priority + (bool ? -1 : 1)});
      var canChange = A2(canIncreaseDecrease,t,bool);
      return canChange ? _U.list([A2($Html$Events.onClick,
      updates.address,
      newTodo)]) : _U.list([]);
   });
   var increaseDecreaseButton = F2(function (t,bool) {
      var arrow = function () {
         var _p0 = bool;
         if (_p0 === true) {
               return $Misc.i18Up;
      ...
      var clickHandler = A2(increaseDecreaseClickHandler,t,bool);
      var spanClass = A2(increaseDecreaseSpanClass,t,bool);
      var spanAttrs = A2($List._op["::"],spanClass,clickHandler);
      return A2($Html.span,spanAttrs,_U.list([$Html.text(arrow)]));
   });
   var increaseButton = function (t) {
      return A2(increaseDecreaseButton,t,true);
   };
   var decreaseButton = function (t) {
      return A2(increaseDecreaseButton,t,false);
   };
   var doneButton = function (t) {
      var newTodo = _U.update(t,{done: t.done ? false : true});
      return A2($Html.span,
      _U.list([$Html$Attributes.$class("btn btn-success btn-xs")
              ,A2($Html$Events.onClick,updates.address,newTodo)]),
      _U.list([$Html.text($Misc.i18Done)]));
   };
   var controlButtons = F2(function (toDelete,t) {
      var buttons = _U.eq(t.id,toDelete) ? _U.list([cancelButton
                                                   ,confirmButton(t.id)]) : A2($List.map,
      function (f) {
         return f(t);
      },
      _U.list([increaseButton
              ,decreaseButton
              ,doneButton
              ,deleteButton]));
      var space = $Html.text($Util.nbsp);
      return A2($List.intersperse,space,buttons);
   });
   var view = F3(function (lastUpdated,toDelete,t) {
      if (_U.eq(t.id,0)) {
            var updater = A2($Html.div,
            _U.list([$Html$Attributes.$class("input-group input-group-sm")]),
            _U.list([A2($Html.input,
            _U.list([$Html$Attributes.value(t.newDescription)
                    ,$Html$Attributes.type$("text")
                    ,$Html$Attributes.id("description_0")
                    ,$Html$Attributes.$class("form-control")
                    ,$Html$Attributes.size($Misc.maxDesc)
                    ,$Html$Attributes.maxlength($Misc.maxDesc)
                    ,$Html$Attributes.placeholder($Misc.i18NewTodo)
                    ...
                    function (value) {
                       ...
                       {ctor: "_Tuple2",_0: 0,_1: value});
                    })
                    ,A2($Util.onEnter,
                    creates.address,
                    _U.update(t,{description: t.newDescription}))]),
            _U.list([]))]));
            return A2($Html.tr,
            _U.list([]),
            _U.list([A2($Html.td,
            ...
            _U.list([$Html$Attributes.value(t.newDescription)
                    ...
                    function (value) {
                       return A2($Signal.message,
                       descriptions.address,
                       {ctor: "_Tuple2",_0: t.id,_1: value});
                    })
                    ,A2($Html$Events.onBlur,
                    edits.address,
                    {ctor: "_Tuple2",_0: t.id,_1: false})
                    ,A2($Util.onEnter,
                    updates.address,
                    _U.update(t,{description: t.newDescription}))]),
            _U.list([]))]));
            var buttons = A2(controlButtons,toDelete,t);
            var rowClass = _U.eq(lastUpdated,t.id) ? "last-updated" : "";
            var spanAtr = $Html$Attributes.$class(cellClass(t));
            var priority = A2($Html.span,
            _U.list([spanAtr]),
            _U.list([$Html.text(priorityDescription(t))]));
            var description = A2($Html.span,
            _U.list([spanAtr
                    ,A2($Html$Events.onClick,
                    edits.address,
                    {ctor: "_Tuple2",_0: t.id,_1: true})]),
            _U.list([$Html.text(t.description)]));
            return A2($Html.tr,
            ...
            _U.list([A2($Html.td,
                    _U.list([]),
                    _U.list([t.editingDescription ? updater : description]))
                    ,A2($Html.td,
                    _U.list([$Html$Attributes.$class("col-md-2")]),
                    _U.list([priority]))
                    ,A2($Html.td,
                    _U.list([$Html$Attributes.$class("col-md-2 text-center")]),
                    buttons)]));
         }
   });
   var Todo = F6(function (a,b,c,d,e,f) {
   ...
   var createTodo = function (t) {
      ...
      else return $Task.fail($Http.UnexpectedPayload("can\'t create todos without a description"));
   };
   var updateTodo = function (t) {
      ...
      else return $Task.fail($Http.UnexpectedPayload("can\'t update todos unless ID > 0"));
   };
...
Elm.Todos.make = function (_elm) {
   ...
   if (_elm.Todos.values) return _elm.Todos.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Misc.indexAndCreateUrl));
   var view = F3(function (lastUpdated,toDelete,todos) {
      var rows = A2($List.map,
      A2($Todo.view,lastUpdated,toDelete),
      A2($List.sortWith,$Todo.todoCompare,todos));
      return A2($Html.table,
      _U.list([$Html$Attributes.$class("table table-bordered table-striped")]),
      _U.list([A2($Html.tbody,_U.list([]),rows)]));
   });
   return _elm.Todos.values = {_op: _op
                              ,view: view
                              ,getTodos: getTodos};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var view = function (model) {
      var error = function () {
         ...
         if (_p0.ctor === "Just") {
               ...
               return A2($Html.p,
               _U.list([$Html$Attributes.hidden(true)]),
               _U.list([$Html.text("No error")]));
            }
      }();
      return A2($Html.div,
      _U.list([]),
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "AddNewTodo": var _p2 = _p1._0;
           if (_p2.ctor === "Ok") {
                 var _p3 = _p2._0;
                 var todo$ = _U.update(_p3,{newDescription: _p3.description});
                 var newTodo = function (t) {
                    return _U.eq(t.id,0) ? _U.update(t,{newDescription: ""}) : t;
                 };
                 return _U.update(model,
                 {todos: A2($List._op["::"],
                 todo$,
                 A2($List.map,newTodo,model.todos))
                 ,lastUpdated: _p3.id
                 ,error: $Maybe.Nothing});
              } else {
                 return _U.update(model,
                 {error: $Maybe.Just($Basics.toString(_p2._0))});
              }
         case "CancelDelete": return _U.update(model,
           {maybeDelete: 0,lastUpdated: 0,error: $Maybe.Nothing});
         case "ConfirmDelete": var _p4 = _p1._0;
           return _U.update(model,
           {maybeDelete: _p4,lastUpdated: _p4,error: $Maybe.Nothing});
         case "DeleteTodo": var _p5 = _p1._0;
           if (_p5.ctor === "Ok") {
                 return _U.update(model,
                 {todos: A2($List.filter,
                 function (t) {
                    return !_U.eq(t.id,_p5._0);
                 },
                 model.todos)
                 ,lastUpdated: 0
                 ,maybeDelete: 0
                 ...
                 return _U.update(model,
                 {error: $Maybe.Just($Basics.toString(_p5._0))});
              }
         case "EditingDescription": var newTodo = function (t) {
              return _U.update(t,
              {editingDescription: _U.eq(t.id,
              _p1._0._0) ? _p1._0._1 : false});
           };
           return _U.update(model,
           {todos: A2($List.map,newTodo,model.todos)});
         case "UpdateDescription": var newTodo = function (t) {
              return _U.eq(t.id,_p1._0._0) ? _U.update(t,
              {newDescription: _p1._0._1}) : t;
           };
           return _U.update(model,
           {todos: A2($List.map,newTodo,model.todos)});
         case "UpdateTodo": var _p6 = _p1._0;
           if (_p6.ctor === "Ok") {
                 var _p7 = _p6._0;
                 var newTodo = function (t) {
                    return _U.eq(t.id,_p7.id) ? _U.update(_p7,
                    {newDescription: _p7.description}) : t;
                 };
                 return _U.update(model,
                 {todos: A2($List.map,newTodo,model.todos)
                 ,lastUpdated: _p7.id
                 ...
                 return _U.update(model,
                 {error: $Maybe.Just($Basics.toString(_p6._0))});
              }
         default: var _p8 = _p1._0;
           if (_p8.ctor === "Ok") {
                 var newTodo = function (t) {
                    return _U.update(t,{newDescription: t.description});
                 };
                 var todos = A2($List.map,
                 newTodo,
                 A2($List._op["::"],$Todo.exampleTodo,_p8._0));
                 return _U.update(model,{todos: todos,error: $Maybe.Nothing});
              } else {
                 return _U.update(model,
                 {error: $Maybe.Just($Basics.toString(_p8._0))});
              }}
   });
   var SetTodos = function (a) {
      return {ctor: "SetTodos",_0: a};
   };
   var UpdateTodo = function (a) {
      return {ctor: "UpdateTodo",_0: a};
   };
   var UpdateDescription = function (a) {
      return {ctor: "UpdateDescription",_0: a};
   };
   var EditingDescription = function (a) {
      return {ctor: "EditingDescription",_0: a};
   };
   var DeleteTodo = function (a) {
   ...
   var AddNewTodo = function (a) {
      return {ctor: "AddNewTodo",_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   var mailBox = $Signal.mailbox(NoOp);
   var actions = $Signal.mergeMany(_U.list([mailBox.signal
                                           ,A2($Signal.map,EditingDescription,$Todo.edits.signal)
                                           ,A2($Signal.map,UpdateDescription,$Todo.descriptions.signal)
                                           ,A2($Signal.map,ConfirmDelete,$Todo.confirmations.signal)
                                           ,A2($Signal.map,
                                           $Basics.always(CancelDelete),
                                           $Todo.cancellations.signal)]));
   var initTodos = function (todos) {
      return A2($Signal.send,mailBox.address,SetTodos(todos));
   };
   var performIndex = Elm.Native.Task.make(_elm).perform(A2($Task.andThen,
   $Todos.getTodos,
   initTodos));
   var addTodo = function (todo) {
      return A2($Signal.send,mailBox.address,AddNewTodo(todo));
   };
   var performCreates = Elm.Native.Task.make(_elm).performSignal("performCreates",
   A2($Signal.map,
   function (todo) {
      return A2($Task.andThen,$Todo.createTodo(todo),addTodo);
   },
   $Todo.creates.signal));
   var mergeTodo = function (todo) {
      return A2($Signal.send,mailBox.address,UpdateTodo(todo));
   };
   var performUpdates = Elm.Native.Task.make(_elm).performSignal("performUpdates",
   A2($Signal.map,
   function (todo) {
      return A2($Task.andThen,$Todo.updateTodo(todo),mergeTodo);
   },
   $Todo.updates.signal));
   var removeTodo = function (id) {
      return A2($Signal.send,mailBox.address,DeleteTodo(id));
   };
   var performDeletes = Elm.Native.Task.make(_elm).performSignal("performDeletes",
   A2($Signal.map,
   function (id) {
      return A2($Task.andThen,$Todo.deleteTodo(id),removeTodo);
   },
   $Todo.deletes.signal));
   var init = {todos: _U.list([])
              ,lastUpdated: 0
              ,maybeDelete: 0
              ,error: $Maybe.Nothing};
