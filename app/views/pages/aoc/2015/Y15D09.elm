...
answers input =
  let
    ...
    extremes = extreme model
    p1 = List.minimum extremes |> Maybe.withDefault 0 |> toString
    p2 = List.maximum extremes |> Maybe.withDefault 0 |> toString
  in
    join p1 p2
...
extreme model =
  let
    f (c1, c2) = Dict.get (key c1 c2) model.distances |> Maybe.withDefault 0
  in
...
parseLine line model =
  let
    matches = find (AtMost 1) (regex "^(\\w+) to (\\w+) = (\\d+)$") line |> List.map .submatches
  in
    case matches of
      [ [ Just c1, Just c2, Just d ] ] ->
        let
          ...
          cities' =
            if List.member c1 model.cities
              then model.cities
              else c1 :: model.cities
          cities =
            if List.member c2 cities'
              then cities'
              else c2 :: cities'
        in
          { model
