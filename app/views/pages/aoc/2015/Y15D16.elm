...
sue hit model =
  let
    sues = List.filter (\s -> Dict.foldl hit True s.props) model
  in
    case List.length sues of
      0 -> "none"
      1 -> List.map .number sues |> List.head |> Maybe.withDefault 0 |> toString
      _ -> "too many"
...
parseLine line model =
  let
    cs = "(akitas|cars|cats|children|goldfish|perfumes|pomeranians|samoyeds|trees|vizslas): (\\d+)"
    rx = "Sue ([1-9]\\d*): " ++ (List.repeat 3 cs |> String.join ", ")
