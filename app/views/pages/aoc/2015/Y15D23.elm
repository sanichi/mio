...
answers input =
  let
    ...
    model2 = { model1 | registers = Dict.insert "a" 1 model1.registers }
    p1 = run model1 |> get "b" |> toString
    p2 = run model2 |> get "b" |> toString
  in
    join p1 p2
        ...
        let
          model' =
            case inst of
              ...
              Jie r j ->
                { model
                | i = model.i + if (get r model) `rem` 2 == 0 then j else 1
                }
              Jio r j ->
                { model
                | i = model.i + if (get r model) == 1 then j else 1
                }
              NoOp ->
                { model
                | i = model.i + 1
...
parseLine line model =
  ...
  in
    case sm of
      [ [ name, reg, jmp ] ] ->
        let
          n = Maybe.withDefault "" name
          r = Maybe.withDefault "" reg
          j = Maybe.withDefault "" jmp |> String.toInt |> Result.withDefault 0
          i =
            case n of
              "inc" -> Inc r
              "hlf" -> Hlf r
