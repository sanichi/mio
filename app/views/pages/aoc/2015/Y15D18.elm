...
answers input =
  let
    ...
    nm = 100
    m1 = steps nm model
    m2 = stick model |> steps nm
    p1 = count m1 |> toString
    p2 = count m2 |> toString
  in
    join p1 p2
...
sweep oldModel model cell =
  if outside model cell
    then model
    else
      let
        v = newVal oldModel cell
        model = { model | lights = Array.set (index model cell) v model.lights }
        nextCell = next model cell
      in
        sweep oldModel model nextCell


newVal : Model -> Cell -> Bool
newVal model cell =
  if model.stuck && corner model cell
    then True
    else
      let
        n = neighbours model cell
      in
        if query model cell
          then n == 2 || n == 3
          else n == 3
...
query model cell =
  if outside model cell
    then False
    else Array.get (index model cell) model.lights |> Maybe.withDefault False
...
parseInput input =
  let
    ...
    s = Array.length a |> toFloat |> sqrt |> ceiling
    m = s - 1
  in
    Model a s m False
