...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { at: 'hidden', A: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { at: 'mozHidden', A: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { at: 'msHidden', A: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { at: 'webkitHidden', A: 'webkitvisibilitychange' }
		: { at: 'hidden', A: 'visibilitychange' };
}
...
var author$project$Main$initModel = {r: _List_Nil, I: 10, E: false};
...
var author$project$Main$hexToInt = function (hex) {
	var zero = elm$core$Char$toCode('0');
	var digitToInt = F2(
		function (index, _char) {
			return A2(elm$core$Basics$pow, 16, index) * function (code) {
				return code - zero;
			}(
...
var author$project$Main$reverse = function (number) {
	return elm$core$String$reverse(
		elm$core$String$fromInt(number));
};
var author$project$Main$isMagic = function (number) {
	var number_ = author$project$Main$hexToInt(
		author$project$Main$reverse(number));
	return _Utils_eq(number, number_);
};
var author$project$Main$nextNumber = function (num) {
	nextNumber:
	while (true) {
		if (author$project$Main$isMagic(num)) {
			return num;
		} else {
			var $temp$num = num + 1;
			num = $temp$num;
...
var author$project$Ports$waitAMoment = _Platform_outgoingPort(
	'waitAMoment',
	function ($) {
		return elm$json$Json$Encode$null;
	});
var author$project$Main$waitAMoment = author$project$Ports$waitAMoment(0);
	...
	function (msg, model) {
		if (!msg.$) {
			var newModel = _Utils_update(
				model,
				{E: true});
			return _Utils_Tuple2(newModel, author$project$Main$waitAMoment);
		} else {
			var num = author$project$Main$nextNumber(model.I);
			var newModel = _Utils_update(
				model,
				{
					r: A2(elm$core$List$cons, num, model.r),
					I: num + 1,
					E: false
				});
...
var author$project$Main$NextNumber = {$: 0};
...
var author$project$Main$viewButtonRow = function (model) {
	var node = model.E ? A2(
		elm$html$Html$img,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				elm$html$Html$Events$onClick(author$project$Main$NextNumber)
			]),
		_List_fromArray(
			[
				elm$html$Html$text('Next')
			]));
	var display = (elm$core$List$length(model.r) >= 5) ? 'none' : 'table-row';
	var rowStyle = A2(elm$html$Html$Attributes$style, 'display', display);
	return A2(
		elm$html$Html$tr,
		_List_fromArray(
			[rowStyle]),
...
var author$project$Main$viewNumberRow = function (number) {
	return A2(
		elm$html$Html$tr,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('col-xs-6 text-center')
					]),
				_List_fromArray(
					[
						elm$html$Html$text(
...
var author$project$Main$viewRows = function (model) {
	var numberRows = A2(
		elm$core$List$map,
		author$project$Main$viewNumberRow,
		elm$core$List$reverse(model.r));
	var buttonRow = author$project$Main$viewButtonRow(model);
	return _Utils_ap(
		numberRows,
		_List_fromArray(
			[buttonRow]));
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$Attributes$class('col-xs-12 offset-sm-1 col-sm-10 offset-md-2 col-md-8 offset-lg-3 col-lg-6')
							]),
						_List_fromArray(
							[
								A2(
