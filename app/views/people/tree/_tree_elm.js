...
Elm.Family.make = function (_elm) {
   ...
   var default_picture_path = "/images/blank_woman.png";
   var nobody = {id: 0
                ,name: "Initialising..."
                ,years: ""
   ...
   var picturePath = F2(function (picture,person) {
      ...
      A2($Array.get,index,person.pictures));
   });
   var Person = F4(function (a,b,c,d) {
      return {id: a,name: b,years: c,pictures: d};
   });
   return _elm.Family.values = {_op: _op
                               ,Person: Person
                               ,default_picture_path: default_picture_path
                               ,nobody: nobody
...
Elm.Point.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var moveX = F2(function (delta,point) {
      return {ctor: "_Tuple2"
             ,_0: $Basics.fst(point) + delta
             ,_1: $Basics.snd(point)};
   });
   var average = F2(function (p1,p2) {
...
Elm.Box.make = function (_elm) {
   "use strict";
   _elm.Box = _elm.Box || {};
   if (_elm.Box.values) return _elm.Box.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   var line = F2(function (p1,p2) {
      ...
      var path = A2($Graphics$Collage.segment,p1,p2);
      return A2($Graphics$Collage.traced,style,path);
   });
   var rightMost = function (boxes) {
      return A3($List.foldl,
      F2(function (b,x) {
         return _U.cmp(b.rightMost,x) > 0 ? b.rightMost : x;
      }),
      0,
      boxes);
   };
   var leftMost = function (boxes) {
      return A3($List.foldl,
      F2(function (b,x) {
         return _U.cmp(b.leftMost,x) < 0 ? b.leftMost : x;
      }),
      0,
      boxes);
   };
   var box2 = F3(function (isFocus,picture,person) {
      ...
      {ctor: "_Tuple2",_0: px,_1: py},
      $Graphics$Collage.toForm(p));
      return {forms: _U.list([e$$$,p$])
             ,lines: _U.list([])
             ,w: w$$$
             ,h: h$$$
             ,x: 0.0
   ...
   var couple = F3(function (picture,p1,p2) {
      ...
      b1);
      var hb = A2(line,right(m1),left(m2));
      return {forms: A2($List.append,m1.forms,m2.forms)
             ,lines: _U.list([hb])
             ,w: b1.w + b2.w
             ,h: b1.h
             ,x: (b1.x + b2.x) / 2.0
   ...
   var children = F4(function (_p6,h,picture,people) {
      ...
      var l2 = A2(line,
      {ctor: "_Tuple2",_0: b1.x,_1: ym},
      {ctor: "_Tuple2",_0: b2.x,_1: ym});
   ...
   var familyToggler = F2(function (index,families) {
      if (_U.cmp(families,1) < 1) return emptyBox; else {
            var s = A2($Basics._op["++"],
            $Basics.toString(index + 1),
            A2($Basics._op["++"]," of ",$Basics.toString(families)));
            var t = A2($Text.style,$Config.textStyle,$Text.fromString(s));
            ...
            var h$$$ = h$$ + 2 * $Config.margin.y;
            var n = index + 1;
            var n$ = _U.cmp(n,families) < 0 ? n : 0;
            var m = A2($Signal.message,$Config.family.address,n$);
   ...
   var partner = F4(function (_p10,_p9,picture,partner) {
      ...
      return _U.update(b$,
      {forms: A2($List.append,t$.forms,b$.forms)
      ,lines: _U.list([l1,l2])});
   });
   var overflow = F5(function (leftSide,width,height,shift,edge) {
      var extreme = 2 * (shift + edge);
      var offEdge = leftSide ? _U.cmp(extreme,
      0 - width) < 1 : _U.cmp(extreme,width) > -1;
      if (offEdge) {
            var arrow = leftSide ? "☜" : "☞";
            var t = A2($Text.style,
            $Config.largeStyle,
            $Text.fromString(arrow));
            var e = $Graphics$Element.centered(t);
            var w = $Graphics$Element.widthOf(e);
            var w$ = 2 * ($Basics.ceiling($Basics.toFloat(w) / 2.0) + $Config.padding.x);
            var x = function () {
            ...
            var m = A2($Signal.message,$Config.shifts.address,delta);
            var e$ = A2($Graphics$Input.clickable,
            m,
            A4($Graphics$Element.container,
            w$,
            ...
            $Graphics$Collage.toForm(e$));
         } else return emptyForm;
   });
   var Box = F8(function (a,b,c,d,e,f,g,h) {
      return {forms: a
             ,lines: b
             ,w: c
             ,h: d
   ...
   return _elm.Box.values = {_op: _op
                            ,Box: Box
                            ,emptyForm: emptyForm
                            ,emptyBox: emptyBox
                            ,middle: middle
...
Elm.Main.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "UpdateContainer": return _U.update(model,
           {width: _p0._0._0,height: _p0._0._1});
         case "ChangeFocus": return _U.update(model,
           {focus: _p0._0,family: 0,shift: 0});
         case "Shift": return _U.update(model,
           {shift: model.shift + _p0._0});
         case "SwitchFamily": return _U.update(model,{family: _p0._0});
         default: return _U.update(model,{picture: model.picture + 1});}
   });
   var RolloverPictures = {ctor: "RolloverPictures"};
   ...
   var UpdateContainer = function (a) {
      return {ctor: "UpdateContainer",_0: a};
   };
   var actions = $Signal.mergeMany(_U.list([A2($Signal.map,
                                           UpdateContainer,
                                           $Window.dimensions)
                                           ,A2($Signal.map,ChangeFocus,foci)
                                           ,A2($Signal.map,Shift,$Config.shifts.signal)
                                           ,A2($Signal.map,SwitchFamily,$Config.family.signal)
                                           ,A2($Signal.map,
                                           $Basics.always(RolloverPictures),
                                           pictureClock)]));
   var NoOp = {ctor: "NoOp"};
   var view = function (model) {
      ...
      focus.father,
      focus.mother);
      var family = A2($Array.get,model.family,focus.families);
      ...
      shiftedForms,
      _U.list([leftOverflow,rightOverflow]));
   ...
   return _elm.Main.values = {_op: _op
                             ...
                             ,RolloverPictures: RolloverPictures
                             ,update: update
                             ,main: main
                             ,model: model
                             ,actions: actions
                             ,pictureClock: pictureClock};
};
